...
class bbPress_Advanced_Statistics {
    ...
    public function __construct ( $file = '', $version ) {
        ...
        add_action('admin_init', array($this, 'plugin_check'));
        
        // Load our locale once the plugin has finished loading
        add_action('plugins_loaded', array($this, 'loadLocale'));   
    ...
    /**
     * Do some checks when the plugin is initially activated on the blog.
     * Currently checks to see if bbPress is actually installed.
     */
    public function plugin_check() {
        
        // Kill the plugin if bbpress is disabled/not present
        if( !class_exists("bbpress") )
        {                
            deactivate_plugins( plugin_basename( $this->file ) );
            $this->install_error( __('bbPress is required in order to use this plugin. Please install and activate bbPress before continuing.', 'bbpress-improved-statistics-users-online') );
        }
    }
